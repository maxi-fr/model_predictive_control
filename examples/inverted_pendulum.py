...
def _(ca):
    ...
    _p, v, theta, omega = x[0], x[1], x[2], x[3]
...
@app.cell
def _(QuadraticObjective, np, nu, nx):
...
@app.cell
def _(ConstraintList, ControlBoundConstraint, N, StateBoundConstraint, np):
    # Control limits
...
@app.cell
def _(Dynamics, N, OCP, cl, dt, dynamics, np, objective):
    ...
    # Solve the OCP
    X_opt, U_opt, _status = ocp.solve(x0_val)
    return U_opt, X_opt
...
@app.cell
def _(N, U_opt, X_opt, dt, np, plot_controls, plot_states, plt, u_max_val):
