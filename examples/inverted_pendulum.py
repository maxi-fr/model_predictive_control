...
from model_predictive_control.ocp import OCP, quadratic_objective, terminal_quadratic_objective, state_bounds_constraints, control_bounds_constraints
from model_predictive_control.plots import plot_states, plot_controls
...
theta_ddot = (-u[0] * cos_theta - m * l * omega**2 * sin_theta * cos_theta + (M + m) * g * sin_theta) / (l * denominator)
...
ocp.setup(
    ...
    solver="ipopt",
    solver_opts={"print_level": 5, "max_iter": 1000}
)
...
# Plot Control Force
plot_controls(time, U_opt, indices=[0], labels=['Control Force (u) [N]'], fig=fig, ax=axs[2], ylabel='Control Force', bounds=[(-u_max_val, u_max_val)])
